...
本指南详细说明Phase 3的两种HTML预览模式：
- **模式B**：Vital Geometry 风格（默认，适合清新几何审美）
- **模式C**：杂志信息卡风格（可选，适合强视觉冲击力内容）
...
| 场景 | 推荐模式 |
|------|---------| 
| **默认输出**（Phase 2.5 完成后自动执行） | **模式A（双风格提示词）** |
| 需要整体排版预览，清新几何风 | 模式B（Vital Geometry HTML） |
| 需要高视觉冲击力、杂志质感预览 | 模式C（Info Card HTML） |
| 开户推广、热点营销类内容 HTML 预览 | 模式C（Info Card HTML） |
| 知识科普、生活理财类内容 HTML 预览 | 模式B（Vital Geometry HTML） |

---

## 模式B：Vital Geometry HTML预览（默认）
...
**模板文件**：`references/html-templates/info-card-template.html`

**设计风格**：瑞士国际主义 + 杂志编辑美学（info-card-designer 风格）
...
> **每张信息卡必须严格遵循小红书 3:4 宽高比**，这是不可妥协的硬性约束。

```css
/* 模板核心 CSS —— 不得删除或修改此规则 */
.info-card {
    width: 100%;
    aspect-ratio: 3 / 4;       /* 小红书标准比例 */
    display: flex;
    flex-direction: column;
    overflow: hidden;           /* 内容不溢出卡片边界 */
}
.card-body {
    flex: 1;
    min-height: 0;             /* 允许内容收缩，不撑破比例 */
    display: flex;
    flex-direction: column;
    overflow: hidden;
}
.card-footer {
    margin-top: auto;          /* footer 始终贴底 */
    flex-shrink: 0;
}
```

**内容适配原则**：当卡片内容较多时，通过缩减字号（`font-size`）、行高（`line-height`）、间距（`gap`/`margin`/`padding`）来适配固定高度，绝不通过撑高卡片来容纳内容。
...
**2. 杂志编辑美学**
- **Hook 标题模式**：每页主标题改写为 10-20 字的强冲击句式，增强情感共鸣
- 大字号排版（使用 `clamp()` 随卡片宽度自适应）
- 顶部色带（6px 高）区分章节类型与强调色
- 卡片底部品牌标注，保持视觉专业感
...
| 卡片类型 | 顶部色带 | 适用内容 |
|---------|---------|---------|
| 封面卡（COVER） | 纯黑 | 主标题 + 核心价值主张 |
| 章节卡（CHAPTER） | 红色/蓝色/金色 | 知识要点、数据、步骤 |
| 推广卡（PROMO） | 纯黑 | CTA + [券商名称]权益 |

**4. 内容区块组件**

| 组件 | 用途 |
|-----|------|
| `data-grid` 数据网格 | 展示 2-4 个关键数据指标 |
| `point-list` 要点列表 | 带编号/警示标记的结构化清单 |
| `quote-block` 引用块 | 核心金句、左边框强调 |
| `cta-block` 强调框 | 黑底白字，品牌/行动召唤 |

**5. 宽度三档可切换**
- 480px：移动端紧凑布局，适合手机截图分享
- 600px：默认平衡比例（推荐）
- 900px：桌面宽幅，适合公众号封面截图
...
```
读取文件：references/html-templates/info-card-template.html
```
...
**Hook 改写规则**：
- 每页卡片的 `.card-hook` 内容改写为 **10-20 字**的强冲击标题
- 触发情绪：好奇、恐惧错过（FOMO）、反差感、数字冲击
- 禁止官腔，使用口语化表达
...
原文：「ETF定投的优势包括分散风险和门槛低」
Hook：「为什么普通人应该买ETF，而不是股票？」

原文：「坚持长期定投可以获得复利收益」
Hook：「月投500元，20年后比同龄人多38万」
```

#### 步骤4：选择强调色方案

根据内容主题选择各章节卡片的顶部色带颜色：

| 色带颜色 | class | 适用场景 |
|---------|-------|---------|
| 纯黑 `#111111` | `accent-black` | 封面、推广页 |
| 警示红 `#D32F2F` | `accent-red` | 痛点、风险、避坑 |
| 专业蓝 `#1565C0` | `accent-blue` | 方案、技巧、操作 |
| 金色 `#F9A825` | `accent-gold` | 数据、收益、亮点 |
| 成长绿 `#2E7D32` | `accent-green` | 正面结果、成功 |

**推荐配色逻辑**：封面→黑，痛点→红，方案→蓝，数据→金，推广→黑

#### 步骤5：填充模板内容

**填充各卡片的关键字段**：
- `.card-meta`：章节类型标注（COVER / CHAPTER XX / PROMO）
- `.card-hook`：改写后的 Hook 标题（10-20字）
- `.card-subtitle`：正文简短说明（1-2句）
- 数据/要点/引用区块：根据内容类型选用合适组件
- `.card-footer-brand`：统一填写「[券商名称]证券」
- `.card-footer-page`：填写页码（如 `02 / 07`）

**Section 4 标签**：填充 5-8 个话题标签

#### 步骤6：生成 HTML 文件

**文件命名**：
```
小红书笔记预览-信息卡-{YYYY-MM-DD}.html
```
...
**内容完整性**：
- [ ] 所有 10 个标题已填充，第一个默认选中
- [ ] 每张卡片的 Hook 标题已改写（10-20字）
- [ ] 各卡片使用了合适的色带颜色
- [ ] 推广卡包含[券商名称]权益内容
- [ ] 正文内容完整，标签 5-8 个
...
适合展示 4 个关键指标（如利率对比、收益数据）：

```html
<div class="data-grid">
    <div class="data-cell">
        <div class="data-cell-label">年通胀率</div>
        <div class="data-cell-value">3<span class="unit">%</span></div>
    </div>
    <div class="data-cell">
        <div class="data-cell-label">活期利率</div>
        <div class="data-cell-value">0.15<span class="unit">%</span></div>
    </div>
</div>
```
...
<ul class="point-list">
    <li class="point-item">
        <span class="point-marker filled">1</span>
        <div class="point-content">
            <div class="point-title">要点标题（≤12字）</div>
            <div class="point-desc">具体说明（1-2句话）</div>
        </div>
    </li>
</ul>
```

标记样式：`filled`（实心圆）/ 空（描边圆）/ `accent-red filled`（红实心）/ `accent-blue filled`（蓝实心）

#### 引用块（核心金句）

```html
<div class="quote-block">
    <div class="quote-text">"核心金句，10-20字，强冲击"</div>
</div>
```
...
```html
<div class="cta-block">
    <div class="cta-label">NEW CLIENT BENEFITS · 新客专属</div>
    <div class="cta-title">3大权益，限时开放</div>
    <ul class="cta-list">
        <li>新客专属理财 8.18% 预期年化收益率</li>
        <li>Level-2 行情免费 90 天</li>
    </ul>
</div>
```
...
| 维度 | 模式B（Vital Geometry） | 模式C（杂志信息卡） |
|-----|------------------------|-------------------|
| 设计风格 | 动态几何，温暖米色基调 | 瑞士国际主义，强对比黑白 |
| 情感基调 | 亲切、知识感、小红书范 | 专业、冲击力强、媒体感 |
| 适用内容 | 生活理财、知识科普 | 热点营销、开户推广 |
| 卡片展示 | 横向轮播（手机竖屏） | 纵向排列（全部可见） |
| 宽度 | 固定 420px | 三档可选 480/600/900px |
| Hook 改写 | 无（使用原文内容） | 有（自动改写为强冲击句式） |
| 数据展示 | 数据框（标签+值） | 数据网格（2×2 指标对比） |
...
- **页面布局规范**：`references/xiaohongshu-preview-layout-spec.md`
- **卡片设计规范（模式B）**：`references/xiaohongshu-preview-card-spec.md`
- **配色方案（模式B）**：`references/xiaohongshu-preview-color-spec.md`
- **模式C模板**：`references/html-templates/info-card-template.html`
- **Phase 3完整指南**：`references/phase-guides.md#phase-3`
...
**v2.0** (2026-03-17)
- 新增模式C：杂志信息卡风格（info-card-designer 风格）
- 新增宽度三档可切换功能
- 新增 Hook 标题改写指南
- 更新模式选择决策表
