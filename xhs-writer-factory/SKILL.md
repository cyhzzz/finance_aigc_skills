...
```
Step 1: 内容提取（./extracted_notes/）
Step 2: 分析归纳（./analysis/）
Step 3: Skill组装（./output/{品牌名}-xhs/）
Step 4: 双维审核 + 输出确认
```
...
**任务**：对每篇笔记逐一提取全部内容要素，输出到 `./extracted_notes/` 目录
...
**输入**：`./analysis/` 三份归纳文档 + Step 2提取的"品牌信息"
...
1. 询问用户成品Skill输出目录（默认：`./output/{品牌名}-xhs/`）
