...
def cmd_publish(args):
    """
    [DEPRECATED] 直接发布到微信草稿箱已废弃。

    请使用 skills/platform-adaptation/SKILL.md 进行多平台改写，
    生成的 HTML 文件可直接复制粘贴到微信后台编辑器。

    本命令保留用于向后兼容，未来版本将移除。
    """
    import warnings
    warnings.warn(
        "cmd_publish is deprecated. "
        "Use skills/platform-adaptation for multi-platform output.",
        DeprecationWarning,
        stacklevel=2
    )
    cfg = load_config()
...
def main():
    ...
    p_rp.add_argument("-o", "--output", default=None  # Will use system temp directory, help="Output directory for PNG files")
